...
%initialize the bond distribution function
 frame=0;
     for f=1:floor(NuFrame/skip)
      ...
      filenameSave=sprintf('/Volumes/IBI4-ZTanA/ProteinDiffusion/DATA/Langevin_Q2D_SLAR_1024/%s_phi_dot%d/Epsilon%d/q6Sq.dat',Str(np),num(np),QQ(qq));
     save(filenameSave,'-ascii','q6sqAll');
      filenameSave=sprintf('/Volumes/IBI4-ZTanA/ProteinDiffusion/DATA/Langevin_Q2D_SLAR_1024/%s_phi_dot%d/Epsilon%d/q6_colorID.dat',Str(np),num(np),QQ(qq));
